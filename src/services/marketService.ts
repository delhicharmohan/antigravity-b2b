...
export const settleMarket = async (marketId: string, outcome: 'yes' | 'no') => {
    ...
    try {
        ...
        // Optimization: Fetch all needed merchant configs in bulk to avoid N+1 queries
        const winningWagers = wagers.filter(w => w.selection === outcome);
        const uniqueMerchantIds = [...new Set(winningWagers.map(w => w.merchant_id))];
        const merchantConfigs = new Map<string, any>();
        ...
        if (uniqueMerchantIds.length > 0) {
            ...
            merchantsRes.rows.forEach(row => {
                merchantConfigs.set(row.id, row.config);
            });
        }
        ...
        for (const wager of wagers) {
            ...
            if (wager.selection === outcome) {
                ...
                const config = merchantConfigs.get(wager.merchant_id);
