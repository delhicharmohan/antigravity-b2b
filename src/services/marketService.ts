...
export const settleMarket = async (marketId: string, outcome: 'yes' | 'no') => {
    ...
    try {
        ...
        // Optimization: Fetch all merchant configs in one go to avoid N+1 queries
        ...
        if (merchantIds.length > 0) {
            ...
            merchantsRes.rows.forEach((m: any) => merchantMap.set(m.id, m.config));
        }

        for (const wager of wagers) {
            let payout = 0;
            if (wager.selection === outcome) {
                // Fetch merchant rake from map
                const config = merchantMap.get(wager.merchant_id);
